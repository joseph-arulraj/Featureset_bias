...
class FeatureImp:
    ...
    def most_common(self):
        freq_per_patient = self.df.groupby('patientunitstayid')[self.feature_cols].apply(lambda x: x.notnull().sum())
        mean_freq = freq_per_patient.mean()
        ordered_features = mean_freq.sort_values(ascending=False).index.tolist()
        return ordered_features
    
    def xgb(self):
        train_agg = self.df.groupby('patientunitstayid')[self.feature_cols].agg(self.xgb_agg_type)
